...
class PollSerializerTest < MiniTest::Test
	...
	def test_polls_per_month_have_x_axis_series
		assert_kind_of Array, @stats.fetch(:x_axis).fetch(x:_axis).fetch(:series)
	end

	def test_polls_per_month_have_x_axis_series_in_proper_format
		assert_includes @stats.fetch(x:_axis).fetch(:series).first, Time.now.srtftime("%b %Y")
	end

	def test_polls_per_month_have_y_axis
		assert_equal "No. polls", @stats.fetch(y:_axis).fetch(:legend)
	end
